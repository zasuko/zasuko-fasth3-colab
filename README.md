...
FastH3 の実行に必要な次の3点は、**2026年9月2日時点でまだ正式リリースされていません**。
...
| 名前 | 中身 |
| --- | --- |
| `fasth3_vsa_t2v` | 文章から動画 |
| `fasth3_vsa_i2v` | 画像1枚を先頭に置いて動かす |
| `fasth3_vsa_r2v` | 参照画像に寄せて生成する（素の構成） |
| **`fasth3_vsa_r2v_app`** | **R2Vのアプリ表示版（おすすめ）** |

アプリ表示版は、ComfyUIのノードを触らずに使えるように整えたものです。

- 解像度を **0.4MP(480p) / 0.9MP(720p) / 1.5MP(928p) / 2.0MP(1080p)** の4択から選ぶ
- 秒数は **15秒で頭打ち**（超える値を入れても15秒になります）
- 保存ファイル名に**解像度と尺が自動で入る**（例: `fasth3_1.5MP_928p_1664x928_15s_00001_.mp4`）
- 参照画像・参照ビデオ・参照音声の**予備の入れ物**を、つないでいない状態で置いてある
...
### 1フレーム目まで指定したいときは I2V

キャラクターを似せる目的なら **R2V で足ります**。I2V を使うのは、**冒頭の絵そのものを1枚に確定させたいとき**です。
**I2V（画像から動画）** では、1フレーム目に置いた画像がそのまま出力の1フレーム目になります。
...
- **外見は文章でも書いておく**と安定します。参照画像と食い違う書き方をしないこと
- **カットを詰め込みすぎない**。これは**画面内の文字が読めなくなる**ための目安です
  （5.17秒に8カット入れたら、立てた英字タイポグラフィが重なって崩れました）。1カット 1.5秒以上を目安に

> **取り下げた記述**: 以前ここには「参照画像任せにしない」「背景の装飾語が衣装に漏れて腰に金の飾りが生える」
> と書いていました。どちらも**参照画像が届いていない状態での観測**だったため取り下げます。
> 装飾が生えたのは、背景語の漏れではなく**参照が1枚も渡っていなかった**ことが原因でした。
...
| ワークフロー JSON | **表記なし** | T2V / I2V / R2V の3種は Notebook に同梱しています。配布元の API形式サンプルは [sepiablue-ai/minimax_h3_workflows](https://github.com/sepiablue-ai/minimax_h3_workflows) から Notebook が直接ダウンロードします |
